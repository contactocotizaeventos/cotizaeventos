  ...
  if (request.method === "POST") {
    ...
    if (action === "aprobar") {
      ...
      const slugBase = (sol.nombre || "proveedor")
        ...
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "");
      const wa = sol.whatsapp || "";
      const last4 = wa.slice(-4) || String(Date.now()).slice(-4);
      let slug = slugBase + "-" + last4;
      ...
      if (existing && existing.length > 0) {
        slug = slugBase + "-" + String(Date.now()).slice(-6);
      }

      // Parse gallery images from comentarios (comma-separated URLs)
      let imagenes = [];
      if (sol.comentarios) {
        imagenes = sol.comentarios.split(",").map(s => s.trim()).filter(s => s.startsWith("http"));
      }
      ...
      const provData = {
        ...
        tagline: sol.tagline || sol.diferenciador || "",
